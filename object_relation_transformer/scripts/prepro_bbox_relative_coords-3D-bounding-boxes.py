...
def get_bbox_relative_coords(params):
    ...
    """
    #replacement for new set:
    coco_ids_to_paths = {str(int(str(file).strip('.jpg'))): os.path.join(img_dir, str(file)) for file in os.listdir(img_dir) if '.jpg' in str(file)}

    print(coco_ids_to_paths)
    ...
    for ind, box_file in enumerate(box_files):
        ...
        height, width, depth = img_array.shape[:3]
        box=np.load(box_file)#[:,4:]
        print(box.shape)
        print(box[0])
        print(width,height,depth)
        relative_box= box/np.array([width, height, depth, width, height, depth])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--image_root', type=str, default='../SentencesNYUv2_toolbox/data/images',
